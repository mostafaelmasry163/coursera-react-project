...
class Contactus extends Component{
  ...
  validate(firstname, lastname, telnum, email){
    ...
    if(this.state.touched.firstname && firstname.length < 3)
      errors.firstname='First Name Incorrect ';
    else if (this.state.touched.firstname && firstname.length >= 10) 
      errors.firstname='First Name Incorrect ';

    if(this.state.touched.lastname && lastname.length < 3)
      errors.lastname='Last Name Incorrect ';
    else if (this.state.touched.lastname && lastname.length >= 10) 
      errors.lastname='Lastt Name Incorrect ';

      const reg=/^\d+$/;
      if(this.state.touched.telnum && !reg.test(telnum))
      errors.telnum='Tel. Number shouldnot containe any characters';

      if(this.state.touched.email && email.split('').filter(x => x==='@').length !==1)
      errors.email='Email shoud contain a @ sign';
    ...
    return(
        <div className="container">
          ...
          <div className="btn-group" role="group">
            <a role="button" href="tel:+85212345678" className="btn btn-primary"
              ...
              <Form onSubmit={this.handleSubmit}>
                ...
                <FormGroup row>
                  <Col md={{size:6, offset:2}}>
                    <FormGroup check>
                      <Label check>
                        <Input type="checkbox" name="agree" checked={this.state.agree} onChange={this.handleInputChange} /> {' '} <i>May we contact you?</i>
                      </Label>
                    </FormGroup>
                  </Col>
                ...
                <FormGroup row>
                  ...
                  <Col md={10}>
                    <Input type="textarea" id="message" name="message" placeholder="Type Your Feedback" rows="12" value={this.state.message} onChange={this.handleInputChange} />
                  </Col>
                </FormGroup>
                <FormGroup>
